...
export const ANNIVERSARY_DATE = new Date("2022-07-28T00:00:00");

export function getTimeTogether() {
  const now = new Date();
  
  const years = differenceInYears(now, ANNIVERSARY_DATE);
  const months = differenceInMonths(now, ANNIVERSARY_DATE) % 12;
  const days = differenceInDays(now, ANNIVERSARY_DATE) % 30; // Approximation for display
  const hours = differenceInHours(now, ANNIVERSARY_DATE) % 24;
  const minutes = differenceInMinutes(now, ANNIVERSARY_DATE) % 60;
  const seconds = differenceInSeconds(now, ANNIVERSARY_DATE) % 60;
...
export function getCurrentPhase() {
  const now = new Date();
  const feb14 = new Date(now.getFullYear(), 1, 14); // Feb is 1
